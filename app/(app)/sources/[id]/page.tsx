...
export default async function SourceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <Card>
        <h2 className="mb-3 text-sm font-semibold text-white">Deals Sent ({source.deals.length})</h2>
      ...
      <Card>
        <h2 className="mb-3 text-sm font-semibold text-white">Contacts ({source.contacts.length})</h2>
          ...
          <div className="space-y-2">
            {source.contacts.map((c) => (
              ...
              >
                ...
                <span className="text-sm text-slate-400">{c.email ?? c.phone ?? ""}</span>
              </Link>
            ))}
          </div>
        )}
